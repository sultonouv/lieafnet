...
def parse_args():
    ...
    parser.add_argument("--dsm-dir", default="data/potsdam/train_dsm")  # ADD THIS
    ...
    parser.add_argument("--output-dsm-dir", default="data/potsdam/train/dsm_1024")  # ADD THIS
...
def get_img_mask_dsm_padded(image, mask, dsm, patch_size, mode):  # MODIFIED
    ...
    pad_mask = albu.PadIfNeeded(min_height=h, min_width=w, position='bottom_right', border_mode=cv2.BORDER_CONSTANT,
                                value=6)(image=mask)
    pad_dsm = albu.PadIfNeeded(min_height=h, min_width=w, position='bottom_right', border_mode=cv2.BORDER_CONSTANT,
                               value=0)(image=dsm)  # ADD THIS
...
def image_augment(image, mask, dsm, patch_size, mode='train', val_scale=1.0):  # MODIFIED
    ...
    print("Image with and height")
    print(image_height)
    print(image_width)

    print("Mask with and height")
    print(mask_height)
    print(mask_width)

    print("dsm with and height")
    print(dsm_height)
    print(dsm_width)
...
def car_aug(image, mask, dsm):  # MODIFIED
    ...
    resize_crop_1 = albu.Compose([albu.Resize(width=int(image.shape[0] * 1.25), height=int(image.shape[1] * 1.25)),
                                  albu.RandomCrop(width=image.shape[0], height=image.shape[1])])(
    ...
    image_resize_crop_1, mask_resize_crop_1, dsm_resize_crop_1 = resize_crop_1['image'], resize_crop_1['masks'][0], resize_crop_1['masks'][1]  # MODIFIED
    image_resize_crop_2, mask_resize_crop_2, dsm_resize_crop_2 = resize_crop_2['image'], resize_crop_2['masks'][0], resize_crop_2['masks'][1]  # MODIFIED
    image_resize_crop_3, mask_resize_crop_3, dsm_resize_crop_3 = resize_crop_3['image'], resize_crop_3['masks'][0], resize_crop_3['masks'][1]  # MODIFIED
    image_resize_crop_4, mask_resize_crop_4, dsm_resize_crop_4 = resize_crop_4['image'], resize_crop_4['masks'][0], resize_crop_4['masks'][1]  # MODIFIED
    ...
    dsm_list = [dsm, dsm_resize_crop_1, dsm_resize_crop_2, dsm_resize_crop_3,
                dsm_resize_crop_4, dsm_vflip, dsm_hflip, dsm_rotate]  # ADD THIS

    return image_list, mask_list, dsm_list  # MODIFIED


def patch_format(inp):
    (img_path, mask_path, dsm_path, imgs_output_dir, masks_output_dir, dsms_output_dir, eroded, gt, rgb_image,
     mode, val_scale, split_size, stride) = inp  # MODIFIED
    
    img_filename = os.path.basename(img_path)
    mask_filename = os.path.basename(mask_path)
    dsm_filename = os.path.basename(dsm_path)  # ADD THIS
    ...
    # DSM files - assuming they have _dsm.tif suffix
    dsm_path = dsm_path + '_dsm.jpg'  # ADD THIS - adjust suffix as needed
    ...
    image_list, mask_list, dsm_list = image_augment(image=img.copy(), mask=mask.copy(), dsm=dsm.copy(),
                                                      patch_size=split_size, val_scale=val_scale, mode=mode)  # MODIFIED
    ...
    for m in range(len(image_list)):
        ...
        for y in range(0, img.shape[0], stride):
            for x in range(0, img.shape[1], stride):
                ...
                dsm_tile_cut = dsm[y:y + split_size, x:x + split_size]  # ADD THIS
                
                img_tile, mask_tile, dsm_tile = img_tile_cut, mask_tile_cut, dsm_tile_cut  # MODIFIED
                ...
                if img_tile.shape[0] == split_size and img_tile.shape[1] == split_size \
                    ...
                    if cf[4] > 1.0 and mode == 'train':  # ignore car_aug, no improvement
                        car_imgs, car_masks, car_dsms = car_aug(img_tile, mask_tile, dsm_tile)  # MODIFIED
                        for i in range(len(car_imgs)):
                            ...
                            cv2.imwrite(out_dsm_path, car_dsms[i])  # ADD THIS - saves as uint8
                    else:
                        ...
                        cv2.imwrite(out_dsm_path, dsm_tile)  # ADD THIS - saves as uint8
...
if __name__ == "__main__":
    ...
    dsm_paths_raw = glob.glob(os.path.join(dsms_dir, "*.jpg"))  # ADD THIS
    dsm_paths = [p[:-8] for p in dsm_paths_raw]  # ADD THIS - adjust based on your naming: top_potsdam_2_10_dsm.jpg -> top_potsdam_2_10
    ...
    inp = [(img_path, mask_path, dsm_path, imgs_output_dir, masks_output_dir, dsms_output_dir, eroded, gt, rgb_image,
