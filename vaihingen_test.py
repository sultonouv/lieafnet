...
def main():
    ...
    device = f'cuda:{config.gpus[0]}' if getattr(config, 'gpus', None) else 'cuda:0'
    model = Supervision_Train.load_from_checkpoint(os.path.join(config.weights_path, config.test_weights_name+'.ckpt'),  config=config, map_location=device)
    model.to(device)
    model.eval()
