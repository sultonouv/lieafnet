...
class Compose(object):
    ...
    def __call__(self, img, mask, dsm=None):
        if dsm is not None:
            assert img.size == mask.size == dsm.size
            for t in self.transforms:
...
class RandomCrop(object):
    ...
    def __call__(self, img, mask, dsm=None, centroid=None):
        assert img.size == mask.size
        if dsm is not None:
        ...
        th, tw = self.size
        if w == tw and h == th:
            return (img, mask, dsm) if dsm is not None else (img, mask)
        ...
        if self.nopad:
            ...
            if pad_h or pad_w:
                ...
                if dsm is not None:
                    dsm = ImageOps.expand(dsm, border=border, fill=0)
                w, h = img.size
...
class SmartCropV1(object):
    ...
    def __call__(self, img, mask, dsm=None):
        ...
        while True:
            if dsm is not None:
                img_crop, mask_crop, dsm_crop = self.crop(img.copy(), mask.copy(), dsm.copy())
            else:
                img_crop, mask_crop = self.crop(img.copy(), mask.copy())
