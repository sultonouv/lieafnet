...
ORIGIN_IMG_SIZE = (1024, 1024)  # CHANGED from 1024
INPUT_IMG_SIZE = (1024, 1024)   # CHANGED from 1024
TEST_IMG_SIZE = (1024, 1024)    # CHANGED from 1024
...
def train_aug(img, mask, dsm):  # MODIFIED - added dsm
    crop_aug = Compose([RandomScale(scale_list=[0.75, 1.0, 1.25, 1.5], mode='value'),
                        SmartCropV1(crop_size=512, max_ratio=0.75, ignore_index=len(CLASSES), nopad=False)])  # CHANGED crop_size to 512
    img, mask, dsm = crop_aug(img, mask, dsm)
    img, mask, dsm = np.array(img), np.array(mask), np.array(dsm)  # MODIFIED
    
    # Apply augmentation to all three
...
def val_aug(img, mask, dsm):  # MODIFIED - added dsm
    img, mask, dsm = np.array(img), np.array(mask), np.array(dsm)  # MODIFIED
...
class PotsdamDataset(Dataset):
    def __init__(self, data_root='data/potsdam/test', mode='val', 
                 img_dir='images_1024', mask_dir='masks_1024', dsm_dir='dsm_1024',  # MODIFIED - changed to 512 and added dsm_dir
                 img_suffix='.tif', mask_suffix='.png', dsm_suffix='.png',  # MODIFIED - added dsm_suffix as .png
    ...
    def __getitem__(self, index):
        ...
        # Concatenate RGB + DSM -> 4 channels
