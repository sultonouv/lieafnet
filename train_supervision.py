...
def get_args():
    ...
    arg("-s", "--seed", type=int, default=42, help="Random seed (default 42)")
...
class Supervision_Train(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Collect any auxiliary losses stored by the model (e.g. B1 ortho loss)
        if hasattr(self.net, 'get_auxiliary_losses'):
            for aux_name, aux_val in self.net.get_auxiliary_losses().items():
                loss = loss + aux_val
                self.log(f'train/aux_{aux_name}', aux_val, prog_bar=False)
